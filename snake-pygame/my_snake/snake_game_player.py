import random
from enum import Enum
from collections import namedtuple


class Direction(Enum):
    RIGHT = 1
    LEFT = 2
    UP = 3
    DOWN = 4

Point = namedtuple('Point', 'x, y')


# colors
WHITE = (255, 255, 255)
RED = (200, 0, 0)
BLUE1 = (0, 0, 255)
BLUE2 = (0, 100, 255)
BLACK = (0,0,0)

BLOCK_SIZE = 20
SPEED = 20

class SnakeGame:
    
    def __init__(self, w = 640, h = 480):
        self.w = w
        self.h = h
        self.reset()
        self.best_score = 0


    def reset(self):
        # init game state
        self.direction = 1
        self.head = Point(self.w+ 2*BLOCK_SIZE + self.w/2, self.h/2)
        self.snake = [self.head, 
                    Point(self.head.x-BLOCK_SIZE, self.head.y), 
                    Point(self.head.x-BLOCK_SIZE*2, self.head.y)]
        self.score = 0
        self.food = None
        self.place_food()

    # helper function
    def place_food(self):
        x = random.randint(0, (self.w - 2*BLOCK_SIZE)//BLOCK_SIZE) * BLOCK_SIZE + self.w + BLOCK_SIZE*2
        y = random.randint(0, (self.h - 2*BLOCK_SIZE)//BLOCK_SIZE) * BLOCK_SIZE
        self.food = [Point(x, y), Point(x+BLOCK_SIZE, y), Point(x, y+BLOCK_SIZE), Point(x+BLOCK_SIZE, y+ BLOCK_SIZE)]
        for fd in self.food:
            if(fd in self.snake):
                self.place_food()
                break
    
    
    def play_step(self):
        # 1. collect user input
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                quit()
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    if self.direction is not Direction.RIGHT:
                        self.direction = Direction.LEFT
                elif event.key == pygame.K_RIGHT:
                    if self.direction is not Direction.LEFT:
                        self.direction = Direction.RIGHT
                elif event.key == pygame.K_UP:
                    if self.direction is not Direction.DOWN:
                        self.direction = Direction.UP
                elif event.key == pygame.K_DOWN:
                    if self.direction is not Direction.UP:
                        self.direction = Direction.DOWN
                # break -> one key input per play_step
                break
        # 2. move
        self._move(self.direction) # update the head
        self.snake.insert(0, self.head)
        # 3. check if game over
        game_over = False
        if self._is_collision():
            game_over = True
            return game_over, self.score
        # 4. place new food or just move
        if self.head == self.food:
            self.score += 1
            # recursive -> helper function
            self._place_food()
            # no pop is done -> list size not reduced -> looks like increasing its size in the game
        else:
            # remove last element of snake
            self.snake.pop()
        # 5. update ui and clock
        self._update_ui()
        self.clock.tick(SPEED) # decide frame update
        # 6. return game over and score
        game_over  = False
        return game_over, self.score

    def is_collision(self):
        # hits boundary
        if self.head.x > self.w*2 + BLOCK_SIZE*2 - BLOCK_SIZE or self.head.x < self.w + BLOCK_SIZE*2 or self.head.y > self.h - BLOCK_SIZE or self.head.y < 0:
            return True
        # hits itself
        # self.snake[0] -> head
        if self.head in self.snake[1:]:
            return True

        return False
    def place_or_move(self):
        if self.head in self.food:
            self.score += 1
            # recursive -> helper function
            self.place_food()
            # no pop is done -> list size not reduced -> looks like increasing its size in the game
        else:
            self.snake.pop()


    def move(self, direction):
        x = self.head.x
        y = self.head.y
        if direction == 1:
            x += BLOCK_SIZE
        elif direction == 2:
            x -= BLOCK_SIZE
        elif direction == 4:
            y += BLOCK_SIZE
        elif direction == 3:
            y -= BLOCK_SIZE
        self.head = Point(x, y)
        self.snake.insert(0, self.head)

    def _update_ui(self):
        self.display.fill(BLACK)

        for pt in self.snake:
            pygame.draw.rect(self.display, BLUE1, pygame.Rect(pt.x, pt.y, BLOCK_SIZE, BLOCK_SIZE))
            pygame.draw.rect(self.display, BLUE2, pygame.Rect(pt.x + 4, pt.y + 4, 12, 12))
        pygame.draw.rect(self.display, RED, pygame.Rect(self.food.x, self.food.y, BLOCK_SIZE, BLOCK_SIZE))

        text = font.render("Score: " + str(self.score), True, WHITE)
        self.display.blit(text, [0,0])
        # update display surface to the screen
        pygame.display.flip()